...
from GAN.networks import ResNetGenerator2, PRDiscriminator2
...
class Apple2OrangeGANModel:
    def __init__(self, is_train=True):
        ...
        # 定义生成器
        self.netG_A = ResNetGenerator2().to(self.device)
        self.netG_B = ResNetGenerator2().to(self.device)

        # 训练所需参数
        if is_train:
            self.model_names += ['D_A', 'D_B']
            # 定义判别器
            self.netD_A = PRDiscriminator2().to(self.device)
            self.netD_B = PRDiscriminator2().to(self.device)
            # 定义损失函数
    ...
    def _backward_G(self, real_A, real_B, fake_A, fake_B, rec_A, rec_B):
        ...
        loss_G.backward()
        return {'loss_G': loss_G, 'loss_G_A': loss_G_A, 'loss_G_B': loss_G_B,
    ...
    def train_a_batch(self, real_A, real_B):
        ...
        # 将损失和图像加入字典
        args_dict['loss_D_A'] = loss_D_A
        args_dict['loss_D_B'] = loss_D_B
