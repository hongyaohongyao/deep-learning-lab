...
class TrainingModel:
    ...
    def __init__(self, start_epoch=-1, base_path=".\\checkpoints", new_model_func=None, env_name='0'):
        ...
        # 开启visdom
        self.viz = Visdom(env='LSTM_%s' % env_name)
        if self.epoch < 0:
            self.viz.close()
    ...
    def training(self,
                 ...
                 training_time=60,  # 训练次数
                 batch_size=512,
                 print_freq=100,  # 训练过程中的打印频率
                 ):
        # 读取训练数据
        ...
        for epoch in range(self.epoch + 1, self.epoch + training_time + 1):
            ...
            for X, y in test_loader:
                ...
                correct += torch.round(out).eq(y).sum().item()
            ...
            for batchIdx, (X, y) in enumerate(train_loader):
                ...
                epoch_process = round(epoch + batchIdx / batchNum, 5)
